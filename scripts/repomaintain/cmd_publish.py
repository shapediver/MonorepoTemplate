...
PublishableComponent = t.TypedDict(
...
def run(
    ...
    global_args = [
        dry_run,
        no_git,
        json.dumps(publishable_components),
    ...
    for c in publishable_components:
        ...
        cmd = (
            "npm publish" + (" --dry-run" if dry_run else "") + " --scope @shapediver"
            " --access public"
            " --registry "
        )

        # Publish to GitHub if requested.
        github_registry = next(
            (r for r in registries if r["name"] == REGISTRY_GITHUB), None
        )
        if github_registry is not None:
            ...
            if not package_version_exists(root, c, github_registry):
                ...
                echo(
                    f"Skipping publishing step since the package already exists.", "wrn"
                )
            else:
...
"""
        ...
        if npm_registry is not None:
            ...
            if not package_version_exists(root, c, npm_registry):
                ...
                link_npmrc_file(
                    root, [c["component"]], must_exist=True, remove_registries=True
                )
...
def ask_user_for_components_and_version(
    ...
    def ask_for_new_version(old_version: str, cmp_name: t.Optional[str]) -> str:
            ...
            [
                {
                    ...
                    "choices": [
                        ...
                        str(v.next_minor()),
                        str(v.next_major()),
                        {"name": "A custom version.", "value": "custom"},
                    ],
                }
            ]
        )
        ...
        if a["version"] == "custom":
            ...
            a = ask_user(
                [{"type": "input", "name": "version", "message": "Custom version:"}]
            )
            ...
            try:
                ...
                echo(
                    f"\nThe custom version you entered got coerced to '{custom_version}'.",
                    "wrn",
                )
            except ValueError:
                raise PrintMessageError(
...
ERROR:
  ...
  The following components and their respective versions have been found:"""
        ...
        selected_components = list(
            filter(lambda c: c["name"] in answers["selection"], public_components)
        )
...
"""
    ...
    if answers["github"]:
        registries.append({"name": REGISTRY_GITHUB, "url": REGISTRY_GITHUB_URL})
    if answers["npm"]:
        registries.append({"name": REGISTRY_NPM, "url": REGISTRY_NPM_URL})
...
def ask_user_and_prepare_commit_and_tags(
    ...
    if config["publish_mode"] == "all":
        ...
        if (
            not always_ask
            and config["publish_tag_name"] is not None
            and len(config["publish_tag_name"]) > 0
        ):
            tag_names.append(f"{config['publish_tag_name']}@{version}")
        else:
            # Ask the user for a custom tag name and store the answer in cli config.
            echo(
                f"A single Git tag will be created for all components ('<name>@{version}')."
            )
    ...
    return [repo.active_branch.path] + [
        repo.create_tag(name).path for name in tag_names
    ]


def ask_user_and_push_to_origin(
    dry_run: bool, repo: git.Repo, to_push: t.List[str]
) -> None:
...
def update_version(
    ...
    forced_updates: t.Dict[str, t.Dict[str, str]] = {
...
def package_version_exists(
    root: str, c: PublishableComponent, registry: Registry
) -> bool:
